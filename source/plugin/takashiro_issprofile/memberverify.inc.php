...
if(submitcheck('modsubmit')){
	...
	if(!empty($members)) {
		...
		if(!empty($moderation['validate']) && is_array($moderation['validate'])) {
			...
			if(!empty($_POST['verifyoption']) && is_array($_POST['verifyoption'])){
				$plugin_member_verify_table = DB::table('plugin_member_verify');
				foreach($validateuids as $uid){
					if(!empty($_POST['verifyoption'][$uid])){
						$vid = intval($_POST['verifyoption'][$uid]);
						DB::query("UPDATE $plugin_member_verify_table SET uid=$uid WHERE id=$vid");
						if(DB::affected_rows() > 0){
							$verifyinfo = DB::fetch_first("SELECT * FROM $plugin_member_verify_table WHERE id=$vid");
							unset($verifyinfo['id'], $verifyinfo['uid'], $verifyinfo['subserial']);
							C::t('common_member_profile')->update($uid, $verifyinfo);
						}
					}
				}
			}
...
if($validatenum) {
	...
	foreach($allvalidate as $uid => $member) {
		...
		foreach($verifyinfolist as $verifyinfo)
			$members.= "<td>姓名：{$verifyinfo['realname']}<br />学校：{$verifyinfo['awardschool']}<br />获奖年份：{$verifyinfo['awardyear']}<br />UID：{$verifyinfo['uid']}<input type=\"radio\" name=\"verifyoption[{$member['uid']}]\" value=\"{$verifyinfo['id']}\" /></td>";
